...
// ANANSI policy note: unlike the other QYVORA tools, this repository forbids
// all environment access (TestNoEnvironmentSecretAccess), so GITHUB_TOKEN is
// deliberately not honoured here; unauthenticated API rate limits apply and
// are surfaced as a clean error when hit.
