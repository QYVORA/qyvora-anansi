...
# it under %LOCALAPPDATA%\Programs\anansi\bin with the anansi icon, adds that
# directory to your user PATH, and creates a Start Menu shortcut — so anansi
...
#
# Options:
#   $env:ANANSI_VERSION    pinned release tag, e.g. v1.0.0 (default: latest)
#   $env:ANANSI_PREFIX     install directory (default: %LOCALAPPDATA%\Programs\anansi)
...
$Bin = "anansi"
$Version = $env:ANANSI_VERSION
$Prefix = $env:ANANSI_PREFIX
if (-not $Prefix) {
    $Prefix = Join-Path $env:LOCALAPPDATA "Programs\anansi"
}
...
function Build-FromSource {
    ...
    if (-not (Test-Path (Join-Path $PWD "go.mod"))) {
        ...
        $tmp = Join-Path $env:TEMP "qyvora-anansi-src"
        if (Test-Path $tmp) { Remove-Item -Recurse -Force $tmp }
        Write-Step "cloning QYVORA/qyvora-anansi..."
        git clone --depth 1 "https://github.com/QYVORA/qyvora-anansi" $tmp | Out-Null
        Set-Location $tmp
    }
...
function Install-Icon {
    ...
    if (-not (Test-Path $ico)) {
        try {
            ...
            $candidates = @(
                (Join-Path $PWD "assets\anansi.ico"),
...
function Install-StartMenuShortcut {
    ...
    $sc.IconLocation = $ico
    $sc.Description = "Attack Surface Intelligence Engine for web recon and OSINT"
    $sc.Save()
...
Write-Result "run 'anansi --help' to get started."
